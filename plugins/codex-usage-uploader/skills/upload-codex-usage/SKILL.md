...
description: Upload this machine's monthly Codex seven-day quota peak from local Codex JSONL session logs to the fixed GitHub reports repository. Use when the user asks to upload, sync, push, report, record, or统计本机某月 Codex 7天额度使用情况.
---
...
- Records only the maximum observed seven-day quota usage for the month and each natural week. Do not report five-hour usage, latest usage percentages, or near-limit status.
- Writes timestamp values as `yyyy-MM-dd HH:mm:ss`; monthly and weekly date boundaries remain `yyyy-MM-dd`.
